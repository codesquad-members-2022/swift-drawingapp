...
| 시작날짜   | 번호    | 내용                                            | 비고                                     |
| ---------- | :------ | ----------------------------------------------- | ---------------------------------------- |
| 2022.03.12 | Step8   | 오브젝트 목록 표시                              |                                          |
| 2022.03.11 | Step7   | 텍스트 추가하기                                 |                                          |
| 2022.03.10 | Step6   | 위치와 크기 속성 변경하기                       |                                          |
| 2022.03.09 | Step5   | 터치와 드래그                                   |                                          |
...
- [x] 하이어라키 뷰 제작
  - [x] 테이블 뷰 제작
  - [x] 테이블 뷰 셀 제작
- [x] 오브젝트 추가 시 테이블 뷰에 추가
- [x] 테이블 뷰 셀 선택 시 해당 오브젝트 선택
- [x] 드로잉 뷰에서 오브젝트 선택/해제 시 하이어라키뷰에서도 선택/해제
- [x] 2초 터치 시 메뉴 띄움, 기능 구현
  - [x] 맨 뒤로 보내기
  - [x] 뒤로 보내기
  - [x] 맨 앞으로 보내기
  - [x] 앞으로 보내기
...
* 오브젝트 생성 시, 왼쪽 하이어라키뷰에 목록이 출력된다
* 하이어라키 cell 선택 시, 해당 오브젝트가 선택된다
...
- [x] LabelModel, LabelView 클래스 구현
- [x] Font 데이터 정의
- [x] Label 추가버튼 구현
- [x] Inspector 버튼, 메뉴 구현
- [x] 기능 동작 테스트
- [x] 추가기능
  - [x] 폰트변경 메뉴 구현
...
* 텍스트를 화면에 드로잉 할 수 있다
* 색상, 투명도, 위치, 크기속성을 동일하게 사용 할 수 있다
* 폰트를 변경 할 수 있다

------

## [Step6] 위치와 크기 속성 변경하기

### 체크리스트

- [x] 드로잉 객체의 위치와 크기를 변경하는 로직 구현
  - [x] 위치 변경 함수 구현
  - [x] 크기 변경 함수 구현
  - [x] Plane에서 위치, 크기변경 로직 구현
- [x] XCTest로 위치와 크기 변경 테스트
- [x] 인스펙터 UI 제작
- [x] 인스펙터 -> ViewController 간 delegate로 이벤트 연결
- [x] MVC동작을 바탕으로 기능 동작 확인

### 핵심기능

* 인스펙터의 위치, 크기 메뉴를 이용하여, 드로잉 뷰의 위치와 크기를 조절 할 수 있다
* 드래그 중일때 현재 이동하는 위치의 값을 볼 수 있다

### 결과화면

<img src="https://user-images.githubusercontent.com/5019378/157801185-5ca34358-a265-4a3f-8128-4f2c823342fe.gif" alt="Simulator Screen Recording - iPad Pro (12 9-inch) (5th generation) - 2022-03-11 at 13 16 14" style="zoom:150%;" />
...
- [x] PanGusture를 이용하여 그려진 오브젝트를 욺직인다
- [x] Drag할 때 선택한 오브젝트의 복사본을 만들어 이동시킨다
- [x] Drag가 완료되면 복사본의 위치에 선택한 오브젝트가 위치하도록 한다

### 핵심기능

* 화면에 그려진 오브젝트를 원하는 위치로 이동 시킬 수 있다

### 결과화면

![Simulator Screen Recording - iPad Pro (12 9-inch) (5th generation) - 2022-03-10 at 08 11 38](https://user-images.githubusercontent.com/5019378/157554970-ca043138-67b3-4f3a-9162-730c506e6ea4.gif)
